...
async fn wait_for_cln_node(client: &mut ClnRpc) {
    for _ in 0..100 {
...
async fn connect_cln_node(client: &mut ClnRpc, id: String, host: Option<String>, port: Option<u16>) {
    client
        .call(Request::Connect(ConnectRequest { id, host, port }))
        .await
...
async fn setup_cln_backend_ready(
    ...
    wait_for_cln_node(&mut client).await;
